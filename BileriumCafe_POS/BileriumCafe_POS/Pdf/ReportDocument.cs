...
namespace BileriumCafe_POS.Pdf
{
    public class ReportDocument : IDocument
    {
        ...
        void ComposeMostPurchasedAddInsItemTable(IContainer container)
        {
            container.Table(table =>
            {
                ...
                // step 3
                foreach (var addInItem in ReportObj.AddInList)
                {
                    table.Cell().Element(CellStyle).Text((ReportObj.AddInList.IndexOf(addInItem) + 1).ToString());
